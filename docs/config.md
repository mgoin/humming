...
| `mma_type` | Can be `mma`, `wgmma`, `mxmma` or `tcgen05`. Since the MMA type affects weight layout, it is classified under LayerConfig. `tcgen05` is an explicit opt-in to the Blackwell tcgen05 kernels: TS mode where the layer is TS-legal (see `LayerConfig.tcgen05_ts_supported`), SS mode otherwise. The TS weight/scale/zero-point layout is specified in [tcgen05_ts_packing.md](tcgen05_ts_packing.md). |
