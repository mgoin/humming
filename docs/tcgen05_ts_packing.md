...
Status: reference implementation + tests landed on `prototype/d-packing`.
This is the packing spec for the TS-mode (TMEM-A) tcgen05 mainloop, written
against the register-layout contract agreed for the TS refactor
(jinzhen-umma-notes.md §4). It is the document to hand Jinzhen for the
production packer: §7 is the interface contract verbatim; everything else is
the derivation and the executable-reference map.
...
| CUDA repack variant | `weight_repack_nk<..., kUseTcgen05Ts=true>` in `humming/include/humming/kernel/process.cuh`, exposed as `ops.repack_weight(..., use_tcgen05_ts=True)` and `prepare_humming_weight(..., use_tcgen05_ts=True)` |
| loader+dequant simulator | `simulate_ts_thread_regs` in `humming/utils/ts_packing.py` |
| tests | `tests/test_tcgen05_ts_packing.py` (39 tests) |

Scope: bf16/fp16 activations (`kNumBitsA == 16`), power-of-2 even-bit B
codes (u4 primary, u8 covered). AWQ-style W4A16 (u4 + bf16 group scales +
u4 zero-points, gs ∈ {64, 128}) is the production target. Odd-bit and
sub-4-bit dtypes: see decision points (§6).
...
`test_ts_register_contract_simulation` walks every (warp, lane, K-chunk)
through this gather + the §2 slot extraction and asserts the contract.
...
`prepare_humming_weight_scale` 8×8 transpose within 64-row blocks +
...
**Bias** (epilogue): unaffected — bias is applied on the output N columns in
the epilogue, whose layout is owned by the TS epilogue work, not the pack.

## 6. Decision points flagged for Jinzhen
...
1. **BlockK > 64**: the B-code pack is 16-K-granular and BlockK-agnostic
   ...
   enters the packed format). The BlockK ≤ 64 constraint in the UMMA
   skeleton came from the *activation* SMEM descriptor
...
5. **Where scale application happens**: this spec only fixes *ownership*
   (lane = row). Whether scales are folded pre-`tcgen05.st` (Swordfish
   pre-applies on the operand; our SS kernel folds in `transform_b` via
   `may_apply_bs_and_zp_on_b`) is a mainloop decision and does not affect
   the streams.
6. **`prepare_humming_weight` gating**: u4/bf16 never takes
   `should_preprocess_for_int2fp` (sign-magnitude) — the TS repack is
   value-preserving for the production path. The CUDA TS branch implements
   the int2fp/zp preprocessing anyway (same code, row-indexed), so u7/u8+zp
   style dtypes inherit it, but only u4/u8 are test-pinned today.

## 7. Interface contract (paste-ready for the production packer)

> **tcgen05-TS register-layout contract (bf16/fp16 activations)**
>
...
> `tests/test_tcgen05_ts_packing.py`.

## 8. Validation matrix (all green on B300, 2026-07-11)

| test | proves |
| --- | --- |
| `test_ts_pack_roundtrip` (6 shapes × {u4,u8}) + MoE | layout bijection |
| `test_ts_register_contract_simulation` ({u4,u8}, every warp/lane/chunk) | loader gather + lop3 slots deliver the contract |
| `test_ts_contract_block_n64` | MmaM=64 tiles (2-warp cover) |
...
| `test_cuda_ts_repack_matches_reference` (6 shapes × {u4,u8}) | CUDA variant ≡ Python reference |
| `test_cuda_ts_repack_group_sizes` (gs 64/128), `_packed_input`, `_moe` | E2E prepare paths |
| `tests/test_tcgen05.py`, `tests/test_tcgen05_dtypes.py`, `tests/test_sm100_smoke.py` | shared repack kernel change did not disturb the shipped SS path |
