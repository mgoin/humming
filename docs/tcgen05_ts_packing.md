# tcgen05 TS-mode weight / scale / zero-point packing

This specifies the packed weight, scale and zero-point layouts the TS-mode
(TMEM-A) tcgen05 mainloop consumes, and derives them from the dequant
instruction sequence and the loader gather they have to fit. §7 states the
resulting contract on its own.
...
The mma.sync packer arranges codes so that after the lop3 dequant each thread's
...
This is the **same** compensation formula the mma.sync pack uses
...
Identical outer shape/tiling to the mma.sync tensor:

```
int32 [ K/16 , N · 16 · kBitsB / 32 ]        (u4: [K/16, 2N])
```

* padded N: multiple of 64; padded K: multiple of 32 (as in the mma.sync
  layout).
* row `c` holds K-chunk `[16c, 16c+16)` for ALL n — g2s slices columns
  `[n_block · BlockN·16·kBitsB/32, …)` per CTA tile unchanged
...
### Contrast with the mma.sync layout

mma.sync layout (u4, decoded and pinned by tests):
...
64-row half-block, so the pack targets the unmodified loader byte-for-byte.
`test_register_contract` asserts this gather plus the §2 slot extraction
against the contract.

`regs_qb` sizing note: the half-group path writes `kBitsB/2` words; the
`regs_qb[2][kBitsB]` declaration in the MMA structs is oversized
...
The mma.sync streams give thread `t` the values for rows `t/4 + 8·f` (the
...
* **Odd-bit dtypes (u3/u5/u7)**: `kLoadHalfGroup` requires an even kBitsB, so
  the half-group gather does not split odd-bit slots; and
  `humming_pack_weight`'s 3/5/6/7-bit re-compression crosses the row boundary
  inside a 32-element group under the TS element order (group =
  [row A 16K | row B 16K]). Both need a dedicated loader path and an in-group
  compression layout before odd-bit TS support. u4/u8 (and by the same
  structure u2/u6/fp4/fp8-as-8bit) are covered.
* **BlockK > 64**: the B-code pack is 16-K-granular and BlockK-agnostic (the
  s2r loader steps `kSmemStride` per K-iter, sectionization never enters the
  packed format). The constraint comes from the *activation* SMEM descriptor
  (`kSwizzleSizeK >= WarpK`), not from the pack.

## 7. Interface contract (16-bit activations)

An MMA-M tile = `min(BlockN, 128)` weight rows (weight rows are humming's "N"
dim). Within a 128-row tile, warp `w` of the `kWarpsPerMmaTile = MmaM/32` warps
covering the tile owns rows `(w % 4) * 32 + lane` (lane = row within the warp's
32-row band).

Per 16-K K-iter, after `dequant()` on the codes the pack produces, thread
`lane` of warp `w` holds its single row's full 16-K chunk in 8 × uint32:
**reg `r` = a pair (K = 2r in lo half, K = 2r+1 in hi half), ascending K** — so
TMEM cell `(lane, col c)` = `W[row, 2c..2c+1]`, the K-major layout TS-mode A
requires. The lop3 (i, i+4) in-word interleave is pre-compensated at pack time:
value slot `s` of each packed word holds K-ascending element
`(s % (V/2))*2 + s/(V/2)`, `V = 32/kBitsB`.

Scales and zero-points follow lane = row ownership: scales `[K/gs, N]` natural
order (thread reads index `row`); zero-points `int32 [K/gs, N*zp_bits/32]`, row
`n` at bits `[(n % (32/zp_bits)) * zp_bits, ...)` of word `n / (32/zp_bits)`.

File format: same tensor shape/tiling as the mma.sync packed weight
(`int32 [K/16, N*16*kBitsB/32]`); inside each 64-row block, 32 slots of `2*W_r`
words: slot `l` = `[row l | row l+32]` half-slots of `W_r` words each,
`W_r = 16*kBitsB/32` — byte-compatible with loader_b's WarpN==32 half-group
gather, so no kernel loader changes.

Everything downstream (tcgen05.st partition, TS-mode mma, epilogue) is on the
kernel side.
