...
class Tcgen05OpClassImpl:
    ...
    def to_cpp_str(self, include_class_name=False):
        lines = [
            ...
            f"static constexpr uint32_t kCTypeBits = {DTYPE_BIT_WIDTH_MAP[self.cd_dtype]};",
            f"static constexpr uint32_t kDTypeBits = {DTYPE_BIT_WIDTH_MAP[self.cd_dtype]};",
            # TS always software-dequants B to A's dtype and issues kind::f16,
            # so tcgen05 is never a native-mixed MMA. mainloop/epilogue arith
            # read MmaOpClass::kNativeMixed unconditionally.
            "static constexpr bool kNativeMixed = false;",
            "",
...
class MmaOpClass:
    @classmethod
    def from_config(
        cls, mma_type, m, n, k, a_dtype, b_dtype, cd_dtype,
        sf_dtype=None, scale_vec=None, warp_shape=None,
    ):
