...
_ACT_BITS = 16  # bf16 / fp16 activations; kPartMmaShapeK = 256 / 16 = 16
...
def _slot_of_element(e: torch.Tensor, values_per_word: int) -> torch.Tensor:
    """lop3 (i, i+4) interleave pre-compensation.

    Value-slot (bit position ``slot * kBits``) that K-ascending element
    ``e`` of a word must be stored at so that dequant reg ``r`` holds
    the (K=2r, K=2r+1) pair.
    """
    half = values_per_word // 2
    return (e % 2) * half + e // 2
...
def pack_weight_tcgen05_ts(codes: torch.Tensor, weight_bits: int = 4) -> torch.Tensor:
    ...
    gathered = codes_v.reshape(*codes_v.shape[:-2], n * 16)[
        ..., flat_idx
    ].reshape(*codes_v.shape[:-2], num_cols, vpw)
...
def unpack_weight_mma_sync(
    ...
    words = packed.view(torch.uint32).to(torch.int64)  # [.., c, col]
    gathered = words[..., col.reshape(-1)].reshape(
        *packed.shape[:-2], num_rows, shape_n, 16
    )
...
def pack_zero_point_tcgen05_ts(
    zero_point: torch.Tensor, weight_bits: int = 4
) -> torch.Tensor:
...
def unpack_zero_point_tcgen05_ts(
    packed: torch.Tensor, shape_n: int, weight_bits: int = 4
) -> torch.Tensor:
    zp_bits = 4 if weight_bits <= 4 else 8
    vpw = 32 // zp_bits
